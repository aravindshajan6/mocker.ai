...
def _next_run(now: datetime) -> datetime:
    target = now.replace(hour=settings.current_affairs_hour_ist, minute=0, second=0, microsecond=0)
    if target <= now:
...
def start() -> asyncio.Task:
    return asyncio.create_task(loop(), name="current-affairs-scheduler")
